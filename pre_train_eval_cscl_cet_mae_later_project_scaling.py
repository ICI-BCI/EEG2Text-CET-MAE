...
def train_mae(train_dataloader, valid_dataloader, model, model_cscl,optimizer, scheduler,tokenizer,
    ...
    for epoch_idx in range(0, num_epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_dataloader)):
            ...
            """"Min-Max Scaling"""
            # mean = torch.mean(inputs_cscl_embeds_batch, dim=(0, 1), keepdim=True)
            # std = torch.std(inputs_cscl_embeds_batch, dim=(0, 1), keepdim=True)
            # inputs_cscl_embeds_batch = (inputs_cscl_embeds_batch - mean) / std
            # Create a new tensor to store the scaled values
            scaled_inputs_cscl_embeds_batch = torch.zeros_like(inputs_cscl_embeds_batch)

            # for i in range(inputs_cscl_embeds_batch.size(0)):
            #     sample = inputs_cscl_embeds_batch[i]
            #     min_vals = torch.min(sample)
            #     max_vals = torch.max(sample)
            #     # Scale the sample and store in the new tensor
            #     scaled_inputs_cscl_embeds_batch[i] = (sample - min_vals) / (max_vals - min_vals)
            for i in range(inputs_cscl_embeds_batch.size(0)):
                sample = inputs_cscl_embeds_batch[i]
                mean_sample = torch.mean(sample, dim=0, keepdim=True)
                std_sample = torch.std(sample, dim=0, keepdim=True)

                # 执行标准化并存储到新的tensor中
                scaled_inputs_cscl_embeds_batch[i] = (sample - mean_sample) / (std_sample + 1e-8)  # 加上一个很小的数以避免除以零

            loss_mae, loss_mlm, loss_c, loss_sim, all_loss = model(scaled_inputs_cscl_embeds_batch,inputs_attn_mask_batch,inputs_attn_mask_invert_bacth,
...
if __name__ == '__main__':
    ...
    desired_weights = [
        ...
        'unify_encoder_layer.norm1.weight',
        'unify_encoder_layer.norm1.bias',
        'unify_encoder_layer.norm2.weight',
        'unify_encoder_layer.norm2.bias',
        'unify_encoder_layer.norm1_e.weight',
        'unify_encoder_layer.norm1_e.bias',
        'unify_encoder_layer.norm2_e.weight',
        'unify_encoder_layer.norm2_e.bias',
        'unify_encoder_layer.norm1_t.weight',
        'unify_encoder_layer.norm1_t.bias',
        'unify_encoder_layer.norm2_t.weight',
        'unify_encoder_layer.norm2_t.bias',
        ...
        'unify_branch.layers.0.norm1.weight',
        'unify_branch.layers.0.norm1.bias',
        'unify_branch.layers.0.norm2.weight',
        'unify_branch.layers.0.norm2.bias',
        'unify_branch.layers.0.norm1_e.weight',
        'unify_branch.layers.0.norm1_e.bias',
        'unify_branch.layers.0.norm2_e.weight',
        'unify_branch.layers.0.norm2_e.bias',
        'unify_branch.layers.0.norm1_t.weight',
        'unify_branch.layers.0.norm1_t.bias',
        'unify_branch.layers.0.norm2_t.weight',
        'unify_branch.layers.0.norm2_t.bias',
        'unify_branch.norm.weight',
        'unify_branch.norm.bias',
        'unify_branch.norm_e.weight',
        'unify_branch.norm_e.bias',
        'unify_branch.norm_t.weight',
        'unify_branch.norm_t.bias',
        'e_branch.layers.0.self_attn.in_proj_weight',
        'e_branch.layers.0.self_attn.in_proj_bias',
        'e_branch.layers.0.self_attn.out_proj.weight',
        'e_branch.layers.0.self_attn.out_proj.bias',
    ...
    model = CETMAE_project_late(multi_heads=8,feedforward_dim=2048,trans_layers=6, device=device)
    model.to(device)
    ...
    early_stopping = EarlyStopper(patience=4, min_delta=0.10)
