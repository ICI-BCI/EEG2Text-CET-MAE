...
from transformers import BertLMHeadModel, BartTokenizer, BartForConditionalGeneration, BartConfig, \
    ...
    MvpTokenizer, MvpConfig, MvpForConditionalGeneration,MvpModel,XLMRobertaTokenizer
from dataset import EEG_dataset_add_sentence_mae as EEG_dataset
# from model_mae import CETMAE_project_late
from model_mae_xlmroberta import CETMAE_project_late_xlmroberta
    ...
    for epoch_idx in range(0, num_epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_dataloader)):
            ...
            loss_mae, loss_mlm, loss_c, loss_sim, all_loss = model(inputs_embeds_batch,inputs_attn_mask_batch,inputs_attn_mask_invert_bacth,
                                                                   mvp_target_tokenized, mask_ratio_e=eeg_mask_ratio,mlm_probability=text_mask_ratio, mlm_loss_weight=mlm_loss_weight, mae_loss_weight=mae_loss_weight, contrast_loss_weight=contrast_loss_weight )
...
if __name__ == '__main__':
    ...
    # if model_name in ['CET-MAE_bart','BrainTranslator', 'BrainTranslatorNaive', 'BrainTranslatorLSTM']:
    #     # tokenizer = BartTokenizer.from_pretrained('facebook/bart-large')
    tokenizer =XLMRobertaTokenizer.from_pretrained('./models/huggingface/xlm-roberta-large')
    ...
    model = CETMAE_project_late_xlmroberta(multi_heads=8,feedforward_dim=2048,trans_layers=6, device=device)
    logger.info("the model is CETMAE_project_late_xlmroberta")
